...
function matchesLawsuit1(assertedConditionText) {
  ...
  return [
    ...
    "interactive images used as links did not describe the content of the link target",
    "links opened new windows without prior warning",
    "product images lacked alternative text",
    "navigation sub-menus with drop-down lists did not announce collapsed state",
    "navigation sub-menus with drop-down lists did not announce expanded state"
...
async function withChallengeGate(page, baseUrl, probeFn) {
...
async function probeHeadingStructure(page, baseUrl) {
  return withChallengeGate(page, baseUrl, async () => {
...
async function probeSkipLink(page, baseUrl) {
  return withChallengeGate(page, baseUrl, async () => {
...
async function probeImageLinkPurpose(page, baseUrl) {
  return withChallengeGate(page, baseUrl, async () => {
    const data = await page.evaluate(() => {
...
async function probeAltText(page, baseUrl) {
  return withChallengeGate(page, baseUrl, async () => {
    const data = await page.evaluate(() => {
...
async function probeNewWindowWarning(page, baseUrl) {
  return withChallengeGate(page, baseUrl, async () => {
    const data = await page.evaluate(() => {
...
async function probeMenuStateAnnouncement(page, baseUrl) {
  return withChallengeGate(page, baseUrl, async () => {
    const data = await page.evaluate(() => {
